...
同一ネットワーク内のL2転送ではMACアドレスを基準としたホストレベルでの転送をしていましたが、
異なるネットワーク間での「**L3転送**」ではIPアドレスを基準としたデータ転送をおこないます。

L2転送はARPを使ったMAC解決をして転送先のアドレスを得ますが、
ネットワークが異なるとL2ブロードキャストが宛先ホストまで届きません。
そのため、送信元ホストは
