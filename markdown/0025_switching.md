...
TCP/IPの世界において、あるホストが別のホストに通信をするには相手をIPアドレスで指定します。
ただ、

あるホストから別のホストに通信をする際は、通常はIPアドレスにより相手を指定します。
ただ、同一のネットワーク内におけるデータの転送はIPアドレスではなく、
イーサネットのアドレスである「**MAC(Media Access Control)アドレス**」を基準としておこなわれます。

イーサネットはL2プロトコルですので、MACアドレスを使ったデータの転送処理は「**L2転送**」と呼ばれています。
このL2転送がどのように行われているかという仕組みと、
転送をするためのMACアドレスを「**ARP(Address Resolution Protocol)**」でどのように得るかというのが本ページの主要なトピックとなります。
