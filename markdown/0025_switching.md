...
そのフレームをそのMACアドレスを持つ機器に転送すればよいことが分かります。
スイッチには複数の機器が接続されていますので、
「どのポートの先にどのMACアドレスを持つ機器が存在するか」という情報を管理しており、
それにしたがって正しい出口からフレームを送信します。
...
まだスイッチが宛先アドレスを学習していない場合はどのポートを使えばいいか分かりません。
...
フラッドされたフレームを受け取った宛先ホストは宛先MACが自分自身なので処理をし、
それ以外のホストは異なる宛先MACあてのフレームは破棄します。

一般的にネットワークの通信は2つのホストがお互いにデータを送り合います。
データをダウンロードするような片方向の通信であっても、
TCP/IPの仕組みが効いてダウンロードしている側も「受け取る準備ができていますよ」
「ここまでデータを受け取りましたよ」と送信側にメッセージを送り返します。
そのため、フレームの宛先から送信元にたいする逆方向の通信も発生しますので、
宛先MACをまだ学習していなくても、すぐに学習されることが期待されます。
