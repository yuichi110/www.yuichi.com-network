...
たとえば、スタティックルートで「10.0.99.0/24へは10.0.2.1を経由する」という設定を行ったうえで、
OSPFで他のルーターから「10.0.99.0/24へは10.0.2.2を経由する」という情報を受け取るかもしれません。

このようなシナリオでどの経路を利用するかということは「**経路選択のアルゴリズム**」に依存します。
上記の場合であればOSPFよりスタティックルートを利用するというルールにもとづいて
「10.0.99.0/24へは10.0.2.1を経由する」というエントリが採用されます。
...
AD値の比較でも優先度が同じであればメトリックの比較をするという流れで採用する経路が決まります。
1つの宛先に対してAD値とメトリックが異なる複数の情報があった場合は、
最も優先される情報のみがルーティングテーブルのエントリとなります。

プレフィックス長とAD値及びメトリックの全てが同じであった場合の挙動は設定次第です。
ただ、一般的にはどれか一つのルートを使うか、
ロードバランス(全て使う)するかのどちらかです。
...
どのような信頼できない情報であっても、どれほど宛先までの通信コストが大きかろうと、
詳細なパスを絶対的に優先します。

たとえばルーティングテーブルのエントリとして「10.0.0.0/8は192.168.0.1経由」
「10.1.0.0/16は192.168.0.2経由」というものがあるとします。
このとき、「10.1.0.1」宛のパケットは上記2つのエントリのどちらにも含まれています。
ただ、プレフィックス長で比較をすると、「192.168.0.1」経由のプレフィックスは「/8」であり、
「192.168.0.2」経由のプレフィックスは「/16」となっています。
「/16」のほうが宛先エントリが詳細(範囲が狭い)ですので、
宛先「10.1.0.1」宛のパケットの場合は「10.1.0.0/16は192.168.0.2経由」というエントリが採用されます。

![image](./0010_image/01.png)

一方、たとえば「10.2.0.1」宛のパケットを受け取った場合は、
「10.1.0.0/16は192.168.0.2経由」というエントリはマッチしないので、
「10.0.0.0/8は192.168.0.1経由」というエントリが採用されます。
...
同じプレフィックス長の経路を複数のソースから学習していた際は、
ソースのAD値を比較することでどのルートを使うかを決めます。
...
「10.0.0.0/8宛のパケットは192.168.0.1」へというスタティックルートと、
「10.0.0.0/8宛のパケットは192.168.0.2」へというOSPFのルートがあるとすれば、
スタティックルートの「10.0.0.0/8宛のパケットは192.168.0.1」がルーティングテーブルのエントリとして採用されます。
重複するOSPFの経路情報はルーティングテーブルには乗りません。

なお、AD値の比較よりプレフィックス長の比較の優先度が高いため、
上記に加えてOSPFで「10.1.0.0/16宛のパケットは192.168.0.2へ」という情報を学べば、
この/16のルートはルーティングテーブルのエントリに記載され、ルーティングに利用されます。
