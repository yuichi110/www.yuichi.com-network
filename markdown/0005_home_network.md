...
自宅で使うルーターはインターネットと自宅のネットワークを繋げるという役割を持っている。
つまりルーターは「ネットワークとネットワークを繋げる」という仕事をしている。
インターネットは多数のネットワークの集合体で、世界中に散らばるルーターがネットワークを接続している。

自分のPCからYahooのウェブページを閲覧するには、Yahooのサーバーとデータのやりとりをする。
インターネットでのやりとりも郵便などと同じようにインターネット上の相手の場所(ネットワーク)を特定する必要があり、
郵便で住所が利用されるようにインターネットでは「IPアドレス」が利用される。

ネットワークの世界で利用されるのはIPだけではなく、TCP/IPという複数のプロトコル(ルール)にしたがって動いている。
TCP/IPを構成するプロトコルはおおまかにTCPとUDP(Layer4)とIP(Layer3)とイーサネット(Layer2)の3階層であり、
実際にはこれを利用するアプリケーションとイーサネットのデータを電気信号や光信号として扱う物理的な機器が存在する。

#### 図: イーサネットとIPによるデータ転送

![image](./0005_image/08.png)

これらのプロトコルはそれぞれ受け持つ仕事が異なっていて、
一番下のイーサネットは「同じネットワーク内のデータ(フレーム)転送」を担当し、
真ん中にあるIPは「異なるネットワーク間のデータ(パケット)転送」を担当している。
スイッチやルーターと呼ばれるネットワーク機器は、この2つのアドレスを使ってデータ転送をしている。

実際にアプリケーションで通信をしあう2つの機器(たとえばPCとYahooのサーバー)は、
イーサーネットとIPに加えて上位にあるTCPやUDPといったプロトコルも利用している。
TCPやUDPはアプリケーションに対して「ソケット」と呼ばれるネットワークを使うためのプログラミングのAPIを提供しており、
アプリケーション間のデータのやりとりに信頼性を持たせる処理などを担当している。

イーサネットはIPのデータを包み、IPはTCP/UDPのデータを包むといった形で、
これらのプロトコルは階層構造のようになっている。


## 自宅ネットワークとインターネット

インターネットの回線を持つ一般的な家では、
無線LANルーターを中心としたネットワークが作られています。
